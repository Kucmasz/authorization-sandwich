...
void MX_X_CUBE_STSE01_Init(void)
{
  ...
  /* Initialize the peripherals and the stsafe components */

  MX_STSE01_MULTI_STEP_AUTHENTICATION_Init();
...
void MX_X_CUBE_STSE01_Process(void)
{
  ...
  /* USER CODE BEGIN SAFEA1xx_Library_Process */

  MX_STSE01_MULTI_STEP_AUTHENTICATION_Process();

}

/**
  * @brief  Initialize the STSAFE MULTI_STEP_AUTHENTICATION Example
  * @retval None
  */
...
void MX_STSE01_MULTI_STEP_AUTHENTICATION_Init(void)
{
  ...
  printf("\n\r---------------------------------------------------------------------------------------------------");
  printf("\n\r- This example illustrates STSAFE-L010 device authentication process using Multi-Step"
         "  approach.               ");
  printf("\n\r- it can be taken as reference for building distant server authentication use cases.               ");
  printf("\n\r---------------------------------------------------------------------------------------------------");

  status = STSE01_Init(SAFEL_010_0);
  if (status == BSP_ERROR_NONE)
  {
    printf("\n\r\n\r\n\r\n\rSTSAFE-L010 I2C initialized successfully\n\r");
  }
  else
  {
    printf("\n\r\n\r\n\r** STSAFE-L010 NOT initialized. ** error code= %d\n\r", stse_ret);
  }
  ...
  stse_handler.io.busID = 1;  /* Needed to use on-board I2C */
...
void MX_STSE01_MULTI_STEP_AUTHENTICATION_Process(void)
{
  ...
  while (HAL_GPIO_ReadPin(SAFEA1_BUTTON_PIN_PORT, SAFEA1_BUTTON_PIN) == GPIO_PIN_SET) {};
}

/**
  * @brief Randomizes the contents of a buffer.
  * @param pBuffer Pointer to the buffer to be randomized.
...
void apps_randomize_buffer(uint8_t *pBuffer, uint16_t buffer_length)
{
  ...
  {
    *(pBuffer + i) = (uint8_t)(rng_generate_random_number() & 0xFFU);
  }
}

/**
  * @brief Generates a random number by using the hardware random number generator (RNG)
  *        to produce a random 32-bit unsigned integer.
  * @return A random 32-bit unsigned integer if successful; otherwise, 0.
  */
uint32_t rng_generate_random_number(void)
{
...
/**
  ...
  * @param buffer Pointer to the buffer whose contents are to be printed.
